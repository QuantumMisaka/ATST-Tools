...
saved by QuantumMisaka in 2023-12-06 from ase-abacus and modified.
"""
...
class MinModeAtoms:
    ...
    *eigenmode*. Furthermore, the original configuration of the Atoms
    ...
    def __init__(self, atoms, control=None, eigenmodes=None,
        ...
        # fixed by copilot with QuantumMisaka
...
class MinModeTranslate(Optimizer):
    ...
    def __init__(self, atoms: MinModeAtoms, logfile='-', trajectory=None, dump_calc=True):
        Optimizer.__init__(self, atoms, None, logfile, trajectory)

        self.control = atoms.get_control()
        self.dump_calc = dump_calc # by QuantumMisaka
    ...
    def dump_properties_traj(self, atoms, dump_dir="Dump_DIMER"):
        """Dump calculation trajectory with property information, 
        
        QuantumMisaka in 2023-12-06
        """
        if not os.path.isdir(dump_dir):
            os.mkdir(dump_dir)
        atoms_buffer = atoms.atoms
        extxyz_buffer = f"{dump_dir}/dimer-run.extxyz"
        write_xyz(extxyz_buffer, atoms_buffer, append=True, write_results=True)
        # atoms_buffer.set_constraint()
        traj_buffer = Trajectory(f"{dump_dir}/dimer-run.traj", "a", atoms_buffer, properties=["energy", "forces", "stress"])
        traj_buffer.write()
    ...
    def step(self, f=None):
        ...
        if self.cg_on:
            direction = self.get_cg_direction(direction) # rotation
        ...
        atoms.set_positions(r + step) # translation
